...
def objective(trial, X, y, preprocessor):
    ...
    try:
        return cross_val_score(
            pipeline, X, y, cv=params["training"]["cv"], scoring="roc_auc"
        ).mean()
    except Exception:
        return 0.0
...
def main():
    # 1. Prep
    print("Configuration loaded: config/params.yaml")
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=params["base"]["test_size"], stratify=y, random_state=42
    )
    ...
    study.optimize(
        lambda t: objective(t, X_train, y_train, preprocessor),
